  ...
  projects: [
    {
      name: 'Dska',
      summary:
        'Более удобная альтернатива официальному порталу электронных госящиков «Datová schránka». Даёт компаниям понятный интерфейс, удобную работу с сообщениями и процессы, которые соответствуют тому, как они реально работают изо дня в день.',
    ...
    },
    {
      name: 'Ombra',
      summary:
        'Внутренняя админ-панель, которую я сделал для игровой студии, чтобы управлять продуктами и подписками. Цель была в том, чтобы ежедневные задачи решались быстро и их было легко держать под контролем.',
    ...
    },
    {
      name: 'Skill',
      summary:
        'Система лицензирования для софта, которому нужен строгий контроль распространения. Лицензии привязаны к конкретному железу и проверяются на бэкенде, а Windows-клиент сделан так, чтобы держать доступ под контролем.',
    ...
    },
    {
      name: 'Stealth Sniper',
      summary:
        'Коммерческий инструмент для автоматизации, который я сделал и продавал. Он выполнял задачи в реальном времени сразу на множестве аккаунтов. Больше всего зависело от тайминга и надёжности, поэтому большая часть работы ушла на то, чтобы он оставался быстрым и стабильным под нагрузкой.',
